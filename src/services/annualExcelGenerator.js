...
function buildSheet1(wb, year, monthlyData, prevData, prevYear, mesesFaltantes) {
  ...
  mergeCell(ws, 1, 1, 1, NUM_COLS, `REPORTE ANUAL DE INGRESOS ${year}`, {
  ...
  ALL_MONTHS.forEach((mm, idx) => {
    ...
    if (d) {
      ...
      if (d.noches != null) {
        ...
        sc(ws, row, 2, '—', {
          font: mkFont(false, 10, C.FONT_MUTED),
          fill: altFill,
          border: bdr,
          align: aC,
        });
  ...
  // B: suma noches
  sf(ws, TOTAL_ROW, 2, `SUM(${sumRange('B')})`, {
    font: fntTotal,
    fill: fillTotal,
    border: bdr,
    align: aR,
  ...
  sf(ws, TOTAL_ROW, 3, `SUM(${sumRange('C')})`, {
    font: fntTotal,
    fill: fillTotal,
    border: bdr,
    align: aR,
  ...
  // D: suma comisión
  sf(ws, TOTAL_ROW, 4, `SUM(${sumRange('D')})`, {
    font: fntTotal,
    fill: fillTotal,
    border: bdr,
    align: aR,
  ...
  // E: suma IVA
  sf(ws, TOTAL_ROW, 5, `SUM(${sumRange('E')})`, {
    font: fntTotal,
    fill: fillTotal,
    border: bdr,
    align: aR,
  ...
  // F: suma ISR
  sf(ws, TOTAL_ROW, 6, `SUM(${sumRange('F')})`, {
    font: fntTotal,
    fill: fillTotal,
    border: bdr,
    align: aR,
  ...
  const totalDeducciones =
    Math.round(
      monthlyData.reduce(
        (s, d) => s + (d.comisionAirbnb || 0) + (d.ivaRetenido || 0) + (d.isrRetenido || 0),
        0
      ) * 100
    ) / 100;
  sc(ws, TOTAL_ROW, 7, totalDeducciones, {
    font: fntTotal,
    fill: fillTotal,
    border: bdr,
    align: aR,
  ...
  // H: suma neto pagado
  sf(ws, TOTAL_ROW, 8, `SUM(${sumRange('H')})`, {
    font: fntTotal,
    fill: fillTotal,
    border: bdr,
    align: aR,
  ...
  // J: suma año anterior
  sf(ws, TOTAL_ROW, 10, `SUM(${sumRange('J')})`, {
    font: fntTotal,
    fill: fillTotal,
    border: bdr,
    align: aR,
  ...
  // K: variación total
  sf(ws, TOTAL_ROW, 11, `H${TOTAL_ROW}-J${TOTAL_ROW}`, {
    font: fntTotal,
    fill: fillTotal,
    border: bdr,
    align: aR,
  ...
  const maxMonth =
    count > 0 ? dataMonths.reduce((a, b) => (b.airbnbTotal > a.airbnbTotal ? b : a)) : null;
  const minMonth =
    count > 0 ? dataMonths.reduce((a, b) => (b.airbnbTotal < a.airbnbTotal ? b : a)) : null;

  const avgMatchRateAll =
    validRates.length > 0 ? validRates.reduce((a, b) => a + b, 0) / validRates.length : 0;
  ...
  const indRows = [
    ...
    [
      'Promedio mensual',
      count > 0 ? totalNeto / count : 0,
      'Noches promedio',
      nochesPromedio,
      NUM_FMT,
      typeof nochesPromedio === 'number' ? '#,##0.0' : null,
    ],
    [
      'Ocupación promedio',
      ocupProm,
      'Meses completos',
      `${12 - mesesFaltantes.length} / 12`,
      typeof ocupProm === 'number' ? PCT_FMT : null,
      null,
    ],
...
function buildSheet2(wb, year, monthlyData, prevData, prevYear) {
  ...
  dataMonths.forEach((d, idx) => {
    ...
    c1.value = d.label;
    c1.font = mkFont(false, 10);
    c1.fill = altFill;
    c1.border = bdr;
    c1.alignment = aL;

    const c2 = ws.getCell(dataRow, 2);
    c2.value = d.airbnbTotal;
    c2.font = mkFont(false, 10);
    c2.fill = altFill;
    c2.border = bdr;
    c2.numFmt = NUM_FMT;
    c2.alignment = aR;
    ...
    c4.value = varPct;
    c4.font = mkFont(false, 10, varColor);
    c4.fill = altFill;
    c4.border = bdr;
    c4.numFmt = varPct != null ? PCT_FMT : '@';
    c4.alignment = aR;
  ...
  noteCell.value =
    'Para insertar gráfica: selecciona esta tabla → Insertar → Gráfico de líneas o columnas';
