    ...
    for (const res of payout.reservations || []) {
      ...
      if (reportMonth && res.checkIn && res.checkOut) {
        ...
        if (nightsPrev > 0 || nightsNext > 0) {
          ...
          d.crossType =
            nightsPrev > 0 && nightsNext > 0 ? 'both' : nightsPrev > 0 ? 'prev' : 'next';
  ...
  for (let d = 1; d <= dias; d++) {
    ...
    if (data?.crossType) {
      ...
      if (data.crossType === 'prev') {
        ...
        noteText =
          `${data.nightsPrevMonth} noche(s) del mes anterior\n` +
...
function buildSheet2(wb, compareResult) {
  ...
  // Anchos
  [20, 20, 30, 16, 16, 16, 16].forEach((w, i) => {
    ws.getColumn(i + 1).width = w;
  });
  ...
  setCell(
    ...
    currentRow,
    1,
    `SOLO EN AIRBNB — PENDIENTES DE DEPÓSITO (${onlyInAirbnb.length})`,
    mkFont(true, 11, C.FONT_WHITE),
    darkFill,
    null,
    hAlign('center')
  ...
  setCell(
    ...
    currentRow,
    1,
    `SOLO EN BANCO — SIN REGISTRO AIRBNB (${onlyInBank.length})`,
    mkFont(true, 11, C.FONT_WHITE),
    darkFill,
    null,
    hAlign('center')
...
function buildSheet3(wb, compareResult, previousYearReport, airbnbData) {
  ...
  const rows = [
    ...
    [
      ...
      prev.payoutsCount || 0,
      (compareResult.matched || []).length + (compareResult.onlyInAirbnb || []).length,
    ],
  ...
  rows.forEach(([concepto, prevVal, currVal], i) => {
    ...
    if (isStr) {
      ...
      const diffFill =
        diff > 0 ? solidFill(C.GREEN_LIGHT) : diff < 0 ? solidFill(C.RED_LIGHT) : fill;
