...
function compareTransactions(airbnbData, bankData, reportMonth = null) {
  ...
  const mesActivo =
    reportMonth || (Array.isArray(airbnbData) ? null : airbnbData.reportMonth) || null;
  ...
  for (const payout of payoutsActivos) {
    ...
    const mejor = candidatos.reduce((prev, curr) => {
      const dP = Math.abs(
        payoutDate.getTime() - new Date(prev.dep.date || prev.dep.liquidationDate || '').getTime()
      );
      const dC = Math.abs(
        payoutDate.getTime() - new Date(curr.dep.date || curr.dep.liquidationDate || '').getTime()
      );
  ...
  const avgDays =
    matched.length > 0 ? matched.reduce((s, m) => s + m.daysDifference, 0) / matched.length : 0;
