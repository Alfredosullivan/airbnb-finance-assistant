...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setData(null);
      try {
        const res = await fetch(`/api/reports/dashboard/${year}?propertyId=${currentProperty.id}`);
        if (!res.ok) return;
  ...
  const cards = [
    ...
    {
      label: 'Ocupación',
      value: `${data.metricas.ocupacion}%`,
      cls: data.metricas.ocupacion >= 60 ? 'green' : '',
    },
  ...
  if (data.variaciones?.ingreso !== null && data.variaciones?.ingreso !== undefined) {
    ...
    const cls =
      data.variaciones.ingreso > 0 ? 'up' : data.variaciones.ingreso < 0 ? 'down' : 'neutral';
  ...
  if (data.variaciones?.noches !== null && data.variaciones?.noches !== undefined) {
    ...
    const cls =
      data.variaciones.noches > 0 ? 'up' : data.variaciones.noches < 0 ? 'down' : 'neutral';
