...
describe('Authentication guard on /api/properties', () => {
  ...
  test('POST / returns 401 when not authenticated', async () => {
    const res = await request(app).post('/api/properties').send({ name: 'Casa Test' });
    expect(res.status).toBe(401);
  });

  test('PUT /:id returns 401 when not authenticated', async () => {
    const res = await request(app).put('/api/properties/1').send({ name: 'Nuevo nombre' });
    expect(res.status).toBe(401);
  });
...
describe('POST /api/properties', () => {
  ...
  test('rejects missing name with 400', async () => {
    ...
    const res = await request(app).post('/api/properties').set('Cookie', cookie).send({});
...
describe('GET /api/properties', () => {
  test('returns empty array when user has no properties', async () => {
    ...
    const res = await request(app).get('/api/properties').set('Cookie', cookie);
  ...
  test('returns all properties belonging to the authenticated user', async () => {
    ...
    await request(app).post('/api/properties').set('Cookie', cookie).send({ name: 'Casa Condesa' });

    await request(app).post('/api/properties').set('Cookie', cookie).send({ name: 'Depa Polanco' });
  ...
  test('user A cannot see user B properties', async () => {
    ...
    const res = await request(app).get('/api/properties').set('Cookie', cookieB);
...
describe('DELETE /api/properties/:id', () => {
  test('cannot delete the only property (must have at least one)', async () => {
    ...
    const deleteRes = await request(app).delete(`/api/properties/${id}`).set('Cookie', cookie);
  ...
  test('deletes a property when the user has more than one', async () => {
    ...
    await request(app).post('/api/properties').set('Cookie', cookie).send({ name: 'Propiedad 2' });
    ...
    // Verify it's gone
    const listRes = await request(app).get('/api/properties').set('Cookie', cookie);
